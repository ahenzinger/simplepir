...
// Ratio between top-level DB and second-level DB
const COMP_RATIO = uint64(64)

// Ratio between amount of communication in the offline
// and online phases
const COMM_RATIO = uint64(10)
...
func (pi *DoublePIR) GetBW(info DBinfo, p Params) {
  ...
  fmt.Printf("\t\tOffline download: %d KB (= %d*%d*%d^2*%d / (8*1024))\n", uint64(offline_download),
             p.delta(), info.x, p.n, p.logq)
...
func (pi *DoublePIR) Setup(DB *Database, shared State, p Params) (State, Msg) {
  ...
  // pack the database more tightly, because online comp is memory-bound
  DB.data.Add(p.p/2)
  DB.Squish()
...
//  H2 := MatrixRand(p.n*p.delta()*info.x, p.n, p.logq, 0)
