...
func RunPIR(pi PIR, DB *Database, p Params, i []uint64) (float64, float64) {
  ...
  //batch_sz := (DB.data.rows/DB.info.ne+num_queries-1) / num_queries * DB.data.cols // TODO: Fix this, so aligns with simplePIR/doublePIR batching!
  bw := float64(0)
...
func BenchmarkDoublePirSingle(b *testing.B) {
  ...
  fmt.Printf("Avg DoublePIR tput (%d, %d), except for first run: %f MB/s\n", log_N, D, avg(tputs))
  fmt.Printf("Std dev of DoublePIR tput (%d, %d), except for first run: %f MB/s\n", log_N, D, stddev(tputs))
}
/*
...
/* -- OUTDATED --
func SearchPirParams(b *testing.B) {
  logN := 20
  clearFile("simple-" + strconv.Itoa(logN) + ".csv")
  clearFile("double-" + strconv.Itoa(logN) + ".csv")

  i := uint64(0) // index to query

  for logn := 9; logn <= 12; logn++ {
    for logL := 10; logL <= 15; logL += 1 {
      ...
      p.PickParams(p.m)
      DB := MakeRandomDB(p.l * p.m, p.logp, p)
      simple_rate, simple_bw := RunPIR(&SimplePIR{}, DB, p, i)
      writeToFile(p, simple_rate, simple_bw, "simple-" + strconv.Itoa(logN) + ".csv")
    }

    for logL := 5; logL <= 10; logL += 1 {
      ...
      p.PickParams(p.m, p.l)
      DB := MakeRandomDB(p.l * p.m, p.logp, p)
      double_rate, double_bw := RunPIR(&DoublePIR{}, DB, p, i)
      writeToFile(p, double_rate, double_bw, "double-" + strconv.Itoa(logN) + ".csv")
    }
  }
}*/
...
func BenchmarkXorBytes(b *testing.B) {
  for j := 0; j<10; j++ {
    ...
    b2[0] = 0x4
    for i := 1; i<len(b1); i++ {
      b1[i] = byte((int(b1[i-1])*7 + 3) % 256)
      b2[i] = byte((int(b2[i-1])*17 + 9) % 256)
    }

    start := time.Now()
    fastxor.Bytes(out, b1, b2)
    printTime(start)
  }
}*/
/*
func BenchmarkXorBlock(b *testing.B) {
  ...
  for j := 0; j<10; j++ {
    x := 0
    y := 16
    for i := 0; i<len(b1); i+=16 {
      fastxor.Block(out[x:y], b1[x:y], b2[x:y])
      x += 16
      y += 16
    }
    printTime(start)
...
// Benchmark XOR performance.
func BenchmarkXorC(b *testing.B) {
  XorTest()
}
